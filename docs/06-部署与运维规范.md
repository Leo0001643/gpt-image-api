...
| dev | https://dev.gpt-image-api.example | compose / k8s-dev | 隔离库 | push to `develop` 自动 |
| staging | https://stg.gpt-image-api.example | k8s-stg | 脱敏快照 | merge to `release/*` 自动 |
| prod | https://gpt-image-api.example | k8s-prod / 单机 compose-prod | 真实 | 手动审批 |
...
services:
  mysql:
    image: mysql:8.0
    container_name: gia-mysql
    restart: always
    command: --default-authentication-plugin=caching_sha2_password
    environment:
      MYSQL_ROOT_PASSWORD: ${MYSQL_ROOT_PASSWORD}
      MYSQL_DATABASE: gpt_image_api
      MYSQL_USER: gia
  ...
  api:
    image: registry.gpt-image-api.example/backend:${TAG}
    container_name: gia-api
    restart: always
    command: ["/app/api"]
  ...
  user-web:
    image: registry.gpt-image-api.example/frontend-user:${TAG}
    container_name: gia-user-web
    restart: always
    expose: ["80"]
    networks: [gia-net]

  admin-web:
    image: registry.gpt-image-api.example/frontend-admin:${TAG}
    container_name: gia-admin-web
    restart: always
    expose: ["80"]
    networks: [gia-net]
...
COPY . .
RUN pnpm --filter @gia/user build
...
server {
  ...
  server_name gpt-image-api.example;

  ssl_certificate     /etc/nginx/certs/gpt-image-api.example.crt;
  ssl_certificate_key /etc/nginx/certs/gpt-image-api.example.key;
...
server {
  ...
  server_name openai.gpt-image-api.example;
  location /v1/ {
...
- 仅 **goose / atlas** 执行，CI 阶段单独跑：
  ```
  goose -dir migrations mysql "${GIA_DB_DSN}" up
  ```
- 必须 **向前兼容**：先发 schema 再发代码，保证滚动期间老/新版本共存可用。

### 7.4 灰度

- API 镜像支持读取 `GIA_GRAY_PCT`，按 `userID hash % 100` 灰度新功能
