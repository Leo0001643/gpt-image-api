...
:root {
  ...
  --gia-300: #94A3FF;
  --gia-400: #4D6BFF;
  --gia-500: #1E3DFF;   /* 电光蓝高光 */
  --gia-600: #002FA7;   /* IKB 主色 */
  --gia-700: #00257F;
  --gia-800: #001B5E;
  --gia-900: #00133F;

  --gia-grad: linear-gradient(135deg, #002FA7 0%, #1E3DFF 100%);
  --gia-grad-soft: linear-gradient(135deg, rgba(0,47,167,.08) 0%, rgba(30,61,255,.12) 100%);
  --gia-glow: 0 0 24px rgba(30, 61, 255, .45);
...
[data-theme="dark"] {
  ...
  --gia-600: #4D6BFF;        /* 深色下提亮主色 */
  --gia-grad: linear-gradient(135deg, #1E3DFF 0%, #4D6BFF 100%);
  --gia-glow: 0 0 32px rgba(77,107,255,.55);
...
export default {
  ...
  theme: {
    extend: {
      ...
      backgroundImage: {
        'gia-grad': 'var(--gia-grad)',
        'gia-soft': 'var(--gia-grad-soft)',
      },
      fontFamily: {
        sans: [
...
| `secondary` | 白底 / 暗底 + 1px gia-600 描边 + gia 字 | 次操作 |
| `ghost` | 透明 + hover gia-100 | 内嵌、表格内操作 |
| `danger` | danger 色 | 删除 |
| `link` | 仅文字 + 下划线 | 跳转 |
...
- 默认描边 `--border-1`；focus 用 `box-shadow: 0 0 0 3px rgba(0,47,167,.15)` + `border: 1px solid var(--gia-600)`
...
- Empty：克莱因蓝插画 + 主操作按钮
- Loading：品牌化 loader（克莱因蓝呼吸方块）；列表 skeleton 用 `animate-pulse + bg-gia-100/10`
...
- 静态文件 CDN 命名：`https://cdn.gpt-image-api.example/static/<hash>.<ext>`
