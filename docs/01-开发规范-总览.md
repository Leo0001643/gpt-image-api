...
> 项目代号：**GPT Image API**（克莱因蓝 AI 生成平台）
...
| 前端 CSS 变量 | `--gia-600` | 见前端规范 |
| 环境变量 | `SCREAMING_SNAKE_CASE`，业务前缀 | `GIA_DB_DSN` |
...
| dev | dev.gpt-image-api.example | 自动部署 develop | 隔离库 |
| staging | stg.gpt-image-api.example | 自动部署 release/* | 脱敏快照 |
| prod | gpt-image-api.example | 手动审批 | 真实数据 |
...
```
GIA_ENV                    # local/dev/staging/prod
GIA_DB_DSN                 # MySQL DSN
GIA_REDIS_ADDR
GIA_REDIS_PASSWORD
GIA_JWT_SECRET             # ≥ 32 字节随机串
GIA_JWT_REFRESH_SECRET
GIA_AES_KEY                # 32 字节，加密账号池密钥
GIA_OPENAI_BASE            # 可被账号池条目覆盖
GIA_GROK_BASE
GIA_ALIPAY_*  / GIA_WXPAY_*
GIA_S3_*                   # 生成结果存储
```
...
5. 全站 **接口签名 + 时间戳**（开放 API），`X-GIA-Sign` + `X-GIA-Ts`，5 分钟窗口。
