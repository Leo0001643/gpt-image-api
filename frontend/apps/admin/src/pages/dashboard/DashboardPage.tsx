  ...
  return (
    <div className="page page-wide space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_320px]">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gia-gradient p-6 text-white shadow-lg">
          ...
          <div className="relative z-10">
            ...
            {/* trend chart */}
            <TrendChart points={data?.trend ?? []} loading={isLoading} />
          </div>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={<Users size={18} />}     iconCls="bg-violet-100 text-violet-600" label="注册用户" value={fmtNumber(data?.users_total)}          sub={`今日新增 ${fmtNumber(data?.users_today)}`} />
        <StatCard icon={<Coins size={18} />}     iconCls="bg-amber-100 text-amber-600"   label="任务积分"  value={fmtPoints(data?.cost_points_today)}      sub={`累计 ${fmtPoints(data?.cost_points_total)}`} />
        <StatCard icon={<Image size={18} />}     iconCls="bg-blue-100 text-blue-600"     label="图片产出"  value={fmtNumber(data?.image_today)}            sub={`累计 ${fmtNumber(data?.image_total)} 张`} />
        <StatCard icon={<Video size={18} />}     iconCls="bg-orange-100 text-orange-600" label="视频产出"  value={fmtNumber(data?.video_today)}            sub={`累计 ${fmtNumber(data?.video_total)} 个`} />
      </div>
...
function TrendChart({ points, loading }: { points: DashboardTrendPoint[]; loading?: boolean }) {
  ...
  const W = 880, H = 120, padX = 16, padY = 12;
  ...
  return (
    <div className="mt-5 rounded-xl border border-white/15 bg-white/8 p-4">
      <div className="mb-3 flex items-center justify-between text-small">
        <div className="flex items-center gap-1.5 font-medium text-white/90"><TrendingUp size={14}/> 近 7 天趋势</div>
        <div className="flex items-center gap-3 text-tiny text-white/65">
          <span className="flex items-center gap-1"><i className="inline-block h-2 w-4 rounded-full bg-white/80"/>生成量</span>
          <span className="flex items-center gap-1"><i className="inline-block h-2 w-4 rounded-full bg-cyan-300"/>积分消耗</span>
        </div>
      </div>
      {loading ? (
        <div className="h-[100px] animate-pulse rounded-lg bg-white/10" />
      ) : (
        <svg viewBox={`0 0 ${W} ${H}`} className="h-[100px] w-full overflow-visible">
          {[0, 1, 2].map((i) => (
            <line key={i} x1={padX} x2={W - padX} y1={padY + i * 42} y2={padY + i * 42}
          ...
          <polygon points={areaG} fill="url(#gAreaGrad)" />
          <polyline points={lineG} fill="none" stroke="rgba(255,255,255,.95)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
          <polyline points={lineC} fill="none" stroke="rgb(103,232,249)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="6,3" />
          {rows.map((p, i) => (
            <g key={p.date}>
              <circle cx={padX + i * step} cy={yG(p.generated)} r="3.5" fill="white" />
              <text x={padX + i * step} y={H - 0} textAnchor="middle" fontSize="16" fill="rgba(255,255,255,.60)">{formatDay(p.date)}</text>
            </g>
          ))}
        </svg>
      )}
...
function StatCard({ icon, iconCls, label, value, sub }: { icon: ReactNode; iconCls: string; label: string; value: string; sub: string }) {
  return (
    <div className="card p-5">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <div className="mt-1.5 text-tiny text-text-tertiary">{sub}</div>
        </div>
        <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${iconCls}`}>{icon}</span>
      </div>
    </div>
